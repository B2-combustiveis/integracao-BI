...
class TipoFormaPagamentoImporter
{
    public function import(array $tipos): array
    {
        ...
        $existentes = $connection->table('tipos_forma_pagamento')
        ...
        $connection->transaction(function () use ($connection, $validos, $novos, $alterados, $agora): void {
            $connection->table('tipos_forma_pagamento')
                ->whereIn('alterdata_id', $validos->pluck('id')->map(fn (mixed $id): string => (string) $id))
                ->update(['ultima_consulta_em' => $agora]);
            if ($novos !== []) {
                $connection->table('tipos_forma_pagamento')->insert($novos);
            }
            if ($alterados !== []) {
                $connection->table('tipos_forma_pagamento')->upsert(
                    $alterados,
                    ['alterdata_id'],
