...
class LoadContasBancariasFromStart extends Command {
 ...
 public function handle(WebPostoCursorSynchronizer $sync,RawResourceImporter $importer): int {
  ...
  $totals=$sync->synchronize(endpoint:'/INTEGRACAO/CONTA',empresaCodigo:$empresa,persist:fn($payload,$parameters)=>$importer->import($payload,$empresa,'contas_bancarias',$parameters),query:['dataInicial'=>'2000-01-01','dataFinal'=>now()->toDateString(),'limite'=>1000],cursor:['initial_value'=>1,'prefer_initial_value'=>!$resume],initialQuery:null,maxPages:max(1,(int)$this->option('pages')),controlKey:$key);
