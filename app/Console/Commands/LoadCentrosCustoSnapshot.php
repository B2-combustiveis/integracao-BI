...
class LoadCentrosCustoSnapshot extends Command
{
    protected $signature = 'webposto:load-centros-custo {empresa=4604}';
    protected $description = 'Padrão 2000: reconcilia o snapshot completo de centros de custo';

    public function handle(WebPostoClient $client, CentroCustoImporter $importer): int
    {
        ...
        try {
            ...
            if (($stored['received'] ?? 0) > 0 && ($stored['inserted'] + $stored['updated'] + $stored['unchanged']) === 0) {
                throw new RuntimeException('O WebPosto não retornou centros de custo válidos.');
            }
            $control->update([
                'status' => 'ok',
                'last_code' => (int) (DB::connection('webposto')->table('centros_custo')->max('centroCustoCodigo') ?? 0),
