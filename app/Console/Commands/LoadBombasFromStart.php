...
class LoadBombasFromStart extends Command {
 ...
 public function handle(WebPostoCursorSynchronizer $sync,RawResourceImporter $importer): int {
  ...
  if(!$resume){$db=DB::connection('webposto');$db->statement('SET FOREIGN_KEY_CHECKS=0');try{$db->table('bombas')->truncate();}finally{$db->statement('SET FOREIGN_KEY_CHECKS=1');}WebPostoSyncControl::where('empresa_codigo',$empresa)->where('endpoint',$key)->delete();}
  $totals=$sync->synchronize(endpoint:'/INTEGRACAO/BOMBA',empresaCodigo:$empresa,persist:fn($payload,$parameters)=>$importer->import($payload,$empresa,'bombas',$parameters),query:['dataInicial'=>'2000-01-01','dataFinal'=>now()->toDateString(),'limite'=>1000],cursor:['initial_value'=>1,'prefer_initial_value'=>!$resume,'single_page'=>true],initialQuery:null,maxPages:max(1,(int)$this->option('pages')),controlKey:$key);
