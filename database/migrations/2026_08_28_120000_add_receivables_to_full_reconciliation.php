...
{
    private const RESOURCES = ['cliente_grupos', 'clientes', 'titulos_receber'];

    public function up(): void
    {
        $this->updateResources(function (array $resources): array {
            $resources = array_values(array_diff($resources, self::RESOURCES));
            $position = array_search('titulos_pagar', $resources, true);
            array_splice($resources, $position === false ? count($resources) : $position + 1, 0, self::RESOURCES);
    ...
    public function down(): void
    {
        $this->updateResources(
            fn (array $resources): array => array_values(array_diff($resources, self::RESOURCES)),
        );
    }
