...
{
    private const CHAIN = ['produto_lmc_lmp', 'bicos', 'lmcs'];

    public function up(): void
    {
        $this->updateResources(function (array $resources): array {
            $resources = array_values(array_diff($resources, self::CHAIN));
            $position = array_search('produto_empresas', $resources, true);
            array_splice($resources, $position === false ? count($resources) : $position + 1, 0, self::CHAIN);
    ...
    public function down(): void
    {
        $this->updateResources(function (array $resources): array {
            ...
            array_splice($resources, $position === false ? count($resources) : $position + 1, 0, ['bicos']);
